	    ...
	    layui.use([ 'jquery', 'layer', 'form', 'table','laydate'  ], function() {
			...
			//渲染数据表格
			 tableIns=table.render({
			    ...
			    ,url:'${rincar}/rent/loadAllRent.action' //数据接口
			...
			$("#doSearch").click(function(){
				...
				tableIns.reload({
					url:"${rincar}/rent/loadAllRent.action?"+params ,
				    page:{
				    	curr:1
				    }
				})
		   ...
		   table.on('tool(rentTable)', function(obj){
			  ...
			  if(layEvent === 'del'){ //删除
				  layer.confirm('真的删除【'+data.rentid+'】这个出租单吗', function(index){
				       //向服务端发送删除指令
				       $.post("${rincar}/rent/deleteRent.action",{rentid:data.rentid},function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
						tableIns.reload();
				       })
			     ...
			     openUpdateRent(data);
				   ...
				   window.location.href="${rincar}/stat/exportRent.action?rentid="+data.rentid;
			   }
			 });
			...
			function openUpdateRent(data){
				mainIndex=layer.open({
					...
					success:function(index){
						...
						url="${rincar}/rent/updateRent.action";
					}
				});
			}
			//保存
