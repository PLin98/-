...
<body class="childrenBody">
	...
	<div  id="rentBar" style="display: none;">
	  <a class="layui-btn layui-btn-xs" lay-event="torentCar">出租</a>
	</div>
	</div>
<!-- 数据表格结束 -->
	    ...
	    layui.use([ 'jquery', 'layer', 'form', 'table','laydate'  ], function() {
			...
			//渲染数据表格
			 tableIns=table.render({
			    ...
			    ,url:'${rincar}/rent/loadRenting.action' //数据接口
			    ...
			    ,cols: [[   //列表数据
			      ...
			      ,{field:'rentflag', title:'出租单状态',align:'center',width:'120',templet:function(d){
			    	  if(d.rentflag=='1'){
			    		  return '<font color=blue>已归还</font>';
			    	  }else if(d.rentflag=='2'){
			    		  return '<font color=green>已预订</font>';
			    	  }else{
			...
			$("#doSearch").click(function(){
				...
				tableIns.reload({
					url:"${rincar}/rent/loadRenting.action?"+params ,
				    page:{
				    	curr:1
				    }
				})
		   ...
		   table.on('tool(rentTable)', function(obj){
			  ...
			  if(layEvent === 'torentCar'){ //出租
				  layer.confirm('真的出租【'+data.carnumber+'】这个汽车吗', function(index){
				       //向服务端发送出租指令
				       $.post("${rincar}/rent/updateRentopername.action",{rentid:data.rentid},function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
						tableIns.reload();
				       })
