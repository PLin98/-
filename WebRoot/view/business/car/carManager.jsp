		   ...
		   table.on('tool(carTable)', function(obj){
			  ...
			  if(layEvent === 'del'){ //删除
				  layer.confirm('真的删除【'+data.carnumber+'】这个车辆吗', function(index){
				       //向服务端发送删除指令
				       $.post("${rincar}/car/deleteCar.action",{carnumber:data.carnumber},function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
			...
			function openAddCar(){
				mainIndex=layer.open({
					...
					success:function(index){
						...
						$("#dataFrm")[0].reset();
						//设置默认图片
						$("#showCarImg").attr("src","${rincar}/file/downloadShowFile.action?path=images/defaultcarimage.jpg")
						$("#carimg").val("images/defaultcarimage.jpg")
						url="${rincar}/car/addCar.action";
			...
			function openUpdateCar(data){
				mainIndex=layer.open({
					...
					success:function(index){
						...
						$("#showCarImg").attr("src","${rincar}/file/downloadShowFile.action?path="+data.carimg);
						url="${rincar}/car/updateCar.action";
			...
			function deleteBatch(){
			    ...
			    layer.confirm('真的删除选中的这些车辆吗', function(index){
				       //向服务端发送删除指令
				       $.post("${rincar}/car/deleteBatchCar.action",params,function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
		    ...
		    upload.render({
		        elem: '#carimgDiv',
		        url: '${rincar}/file/uploadFile.action',
		        method : "post",  //此处是为了演示之用，实际使用中请将此删除，默认用post方式提交
		        acceptMime:'images/*',//规定打开文件选择框时，筛选出的文件类型，值为用逗号隔开的 MIME 类型列表;只显示图片
		        field:"mf",//设定文件域的字段名
		        done: function(res, index, upload){
		            $('#showCarImg').attr('src',"${rincar}/file/downloadShowFile.action?path="+res.data.src);
