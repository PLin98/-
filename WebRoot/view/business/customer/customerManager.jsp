		...
		<div class="layui-form-item">
		   ...
		   <%--  <div class="layui-inline">
		    <label class="layui-form-label">客户住址:</label>
				<div class="layui-input-inline">
				<select name="address" lay-filter="province" class="layui-input">
		...
		<form class="layui-form"  lay-filter="dataFrm" id="dataFrm">
			<div class="layui-form-item">
				<div class="layui-inline">
					<label class="layui-form-label">身份证号:</label>
					<div class="layui-input-inline">
						<input type="text" name="identity" lay-verify="required|identity"   placeholder="请输入身份证号" autocomplete="off"
							class="layui-input">
					</div>
				</div>
				<div class="layui-inline">
	    ...
	    layui.use([ 'jquery', 'layer', 'form', 'table'  ], function() {
			...
			//渲染数据表格
			 tableIns=table.render({
			    ...
			    ,url:'${rincar}/customer/loadAllCustomer.action' //数据接口
			...
			$("#doSearch").click(function(){
				...
				tableIns.reload({
					url:"${rincar}/customer/loadAllCustomer.action?"+params ,
				    page:{
				    	curr:1
				    }
				})
			...
			//导出
				$("#doExport").click(function(){
					var params=$("#searchFrm").serialize();
					window.location.href="${rincar}/stat/exportCustomer.action?"+params;
				});
		   ...
		   table.on('tool(customerTable)', function(obj){
			  ...
			  if(layEvent === 'del'){ //删除
				  layer.confirm('真的删除【'+data.custname+'】这个客户吗', function(index){
				       //向服务端发送删除指令
				       $.post("${rincar}/customer/deleteCustomer.action",{identity:data.identity},function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
			     ...
			     openUpdateCustomer(data);
				   ...
				   layer.confirm('真的重置【'+data.custname+'】这个客户的密码吗', function(index){
				       //向服务端发送删除指令
				       $.post("${rincar}/customer/resetCustomerPwd.action",{identity:data.identity},function(res){
				    	   layer.msg(res.msg);
				       })
				     }); 
			   }
			...
			function openAddCustomer(){
				mainIndex=layer.open({
					...
					success:function(index){
						//清空表单数据       
						$("#dataFrm")[0].reset();
						url="${rincar}/customer/addCustomer.action";
					}
				});
			}
			//打开修改页面
			function openUpdateCustomer(data){
				mainIndex=layer.open({
					...
					success:function(index){
						...
						url="${rincar}/customer/updateCustomer.action";
					}
				});
			}
			//保存
			...
			function deleteBatch(){
			    ...
			    layer.confirm('真的删除选中的这些客户吗', function(index){
				       //向服务端发送删除指令
				       $.post("${rincar}/customer/deleteBatchCustomer.action",params,function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
