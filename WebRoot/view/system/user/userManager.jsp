...
<c:set var="Province" >北京市,天津市,上海市,重庆市,河北省,山西省,辽宁省,吉林省,黑龙江省,
		...
		<form class="layui-form"  lay-filter="dataFrm" id="dataFrm">
			...
			<div class="layui-form-item">
				<div class="layui-inline">
					<label class="layui-form-label">身份证号:</label>
					<div class="layui-input-inline">
						<input type="text" name="identity" lay-verify="required|identity" placeholder="请输入用户身份证号" autocomplete="off"
							class="layui-input">
					</div>
				</div>
	...
	<script type="text/javascript">
	    var tableIns;
	    layui.extend({
			dtree:'${rincar}/resources/layui_ext/dist/dtree'
			...
			//渲染数据表格
			 tableIns=table.render({
			    ...
			    ,url:'${rincar}/user/loadAllUser.action' //数据接口
			...
			$("#doSearch").click(function(){
				...
				tableIns.reload({
					url:"${rincar}/user/loadAllUser.action?"+params
				})
			});
		   ...
		   table.on('tool(userTable)', function(obj){
			  ...
			  if(layEvent === 'del'){ //删除
				  layer.confirm('真的删除【'+data.realname+'】这个用户吗', function(index){
				       //向服务端发送删除指令
				       $.post("${rincar}/user/deleteUser.action",{userid:data.userid},function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
			     ...
			     openUpdateUser(data);
				   ...
				   layer.confirm('真的重置【'+data.realname+'】这个用户的密码吗', function(index){
				       //向服务端发送删除指令
				       $.post("${rincar}/user/resetUserPwd.action",{userid:data.userid},function(res){
				    	   layer.msg(res.msg);
				       })
				     }); 
			...
			function openAddUser(){
				mainIndex=layer.open({
					...
					success:function(index){
						//清空表单数据       
						$("#dataFrm")[0].reset();
						url="${rincar}/user/addUser.action";
					}
				});
			}
			//打开修改页面
			function openUpdateUser(data){
				mainIndex=layer.open({
					...
					success:function(index){
						...
						url="${rincar}/user/updateUser.action";
					}
				});
			}
			//保存
			...
			function deleteBatch(){
			    ...
			    layer.confirm('真的删除选中的这些用户吗', function(index){
				       //向服务端发送删除指令
				       $.post("${rincar}/user/deleteBatchUser.action",params,function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
			...
			function openselectUserRole(data){
				mainIndex=layer.open({
					...
					yes:function(index, layero){
					    ...
					    //保存
					    $.post("${rincar}/user/saveUserRole.action",params,function(obj){
					    	layer.msg(obj.msg);
					    })
					},
					success:function(index){
						//渲染数据表格
						 var roleTableIns=table.render({
						    ...
						    ,url:'${rincar}/user/initUserRole.action?userid='+data.userid //数据接口
						    ,title: '角色列表'//数据导出来的标题
						    ,cellMinWidth:100 //设置列的最小默认宽度
						    ,cols: [[   //列表数据
						     {type: 'checkbox', fixed: 'left'}
