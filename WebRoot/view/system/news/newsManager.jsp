	    ...
	    layui.use([ 'jquery', 'layer', 'form', 'table','laydate','layedit' ], function() {
			...
			//渲染数据表格
			 tableIns=table.render({
			    ...
			    ,url:'${rincar}/news/loadAllNews.action' //数据接口
			...
			$("#doSearch").click(function(){
				...
				tableIns.reload({
					url:"${rincar}/news/loadAllNews.action?"+params,
					page:{curr:1}
				})
			});
		   ...
		   table.on('tool(newsTable)', function(obj){
			  ...
			  if(layEvent === 'del'){ //删除
				  layer.confirm('真的删除【'+data.title+'】这个公告吗', function(index){
				       //向服务端发送删除指令
				       $.post("${rincar}/news/deleteNews.action",{id:data.id},function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
			...
			function openAddNews(){
				mainIndex=layer.open({
					...
					success:function(index){
						...
						//清空表单数据       
						$("#dataFrm")[0].reset();
						url="${rincar}/news/addNews.action";
					}
				});
			}
			...
			function openUpdateNews(data){
				mainIndex=layer.open({
					...
					success:function(index){
						...
						url="${rincar}/news/updateNews.action";
					}
				});
			}
			//保存
			...
			function deleteBatch(){
			    ...
			    layer.confirm('真的删除选中的这些公告吗', function(index){
				       //向服务端发送删除指令
				       $.post("${rincar}/news/deleteBatchNews.action",params,function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
